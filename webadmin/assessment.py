...
def viewassessment(request):
    ...
    return render(request, 'assessment/viewassessments.html',{
            "title":'查看互评',
            'username':username})
...
def goassessment(request):
    ...
    return render(request, 'assessment/goassessment.html',{
            "title":'进行互评',
            'username':username})
...
def go_assessments(request):
    ...
    if request.is_ajax():
        ...
        try:
            ...
            if response == 'false':
                return HttpResponse(json.dumps([response,str(response_num)]), content_type='application/json')
            else:
                if oriresult == '0':
                    assessmentrow.excellent-=1
                elif oriresult == '1':
